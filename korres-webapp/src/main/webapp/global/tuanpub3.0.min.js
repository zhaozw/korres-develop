(function(b) {
	...
	a.use = function() {
		var e = arguments, d = function() {
			var g = arguments[0], h = g[0];
			if (g[0].indexOf("http") == -1) {
				switch (b.getUrl()[2].split(".")[1] + "."
						+ b.getUrl()[2].split(".")[2]) {
				case "tuan800.com":
				...
				callback : function(i) {
					...
					case "object":
						b.each(i, function(n, m) {
							...
							j.OnLoad ? j.OnLoad() : false;
							j.Queue ? j.Queue() : false;
							m.init ? j[m.init].apply(
									b.type(m.t) == "boolean" ? m.t ? this
											: tuanpub.getModule(n) : tuanpub
											.getModule(n), (l)) : false
						});
						break;
					case "function":
			...
			.extend({
				...
				getTime : {
					...
					getDateStr : function(f, x) {
						...
						var f = f || new Date();
						var i = "-", c = "", k = new Date(f);
						k.setTime(f);
						var b = k.getFullYear(), j = b + "".substring(2, 4), g = k
								.getMonth() + 1, n = k.getDate(), v = k
								.getHours(), q = k.getMinutes(), l = k
								.getSeconds();
						var o = g < 10 ? "0" + g : g, u = n < 10 ? "0" + n : n, e = v < 10 ? "0"
								+ v
								: v, w = q < 10 ? "0" + q : q, r = l < 10 ? "0"
								+ l : l;
						var y = {
							Y : b,
							YY : j,
					...
					friendly : function(g) {
						...
						var k = "", w = 1000, p = w * 60, e = p * 60, v = +new Date, c = new Date(), b = c
								.getFullYear(), q = c.getMonth(), i = c
								.getDate(), f = new Date(b, q, i), o = f
								.getTime(), l = v - g;
						if (l < 0) {
							k = ""
						} else {
							if (l <= p * 5) {
								...
								if (l < e) {
									...
									if (l < v - o) {
										var j = new Date(g), d = j.getHours(), u = j
												.getMinutes();
										if (d < 10) {
											d = "0" + d
										}
										...
										var j = new Date(g), h = j
												.getFullYear(), n = j
												.getMonth() + 1, s = j
												.getDate(), d = j.getHours(), u = j
												.getMinutes();
										if (n < 10) {
											n = "0" + n
										}
					...
					countDown : function() {
						if (parseInt(arguments[0]) > 0) {
							var b = this, c = false;
							times = Math
									.floor((arguments[0] - (arguments[1] || (new Date())
											.getTime())) / 1000);
							if (times < 0) {
								c = true;
							...
							return {
								t : c,
								s : times % 60,
								ss : times % 60 < 10 ? "0" + times % 60
										: times % 60,
								m : Math.floor(times / 60) % 60,
								mm : Math.floor(times / 60) % 60 < 10 ? "0"
										+ Math.floor(times / 60) % 60 : Math
										.floor(times / 60) % 60,
								h : Math.floor(times / 3600) % 24,
								hh : Math.floor(times / 3600) % 24 < 10 ? "0"
										+ Math.floor(times / 3600) % 24 : Math
										.floor(times / 3600) % 24,
								D : Math.floor(times / 86400),
								DD : Math.floor(times / 86400) < 10 ? "0"
										+ Math.floor(times / 86400) : Math
										.floor(times / 86400)
							}
						} else {
							return {}
				...
				getCss : function(f, c, i) {
					var e = document.getElementsByTagName("head")[0], b = /loaded|complete/, c = c
							|| {}, i = new Function, d = 0, h;
					...
					function g(k, j, m) {
						...
						if (l.readyState) {
							...
							if (l.onload === null) {
								...
								if (a.browser.core != "msie"
										&& a.browser.core != "opera") {
...
(function(b) {
	...
	a.M = function(c) {
		...
		e.cls = c.cls || "";
		e.diglogWrapper = '<div id="' + e.id
				+ '" class="dialog-wrapper"></div>';
		e.overlay = b.type(c.overlay) === "boolean" ? c.overlay : b
				.type(c.overlay) === "object" ? c.overlay : false;
		e.msgbody = c.msbbody
				|| '{{#close}}<div class="diginfo">{{#msg}}</div>';
		e.tpl = c.tpl || {};
		e.msg = c.msg || "";
	...
	a.M.prototype = {
		Init : function() {
			...
			g.$d = b("#" + g.id);
			g.$d.addClass(g.cls);
			if (!g.overlay && b.browser.name == "msie"
			...
			g.closeBtn = b.type(g.closebtn) == "boolean" && g.closebtn == false ? g.$d
					.find("a .close").hide()
					: g.$d.find(b.type(g.closebtn) === "string" ? g.closebtn
							: "a .close");
		...
		Position : function(e) {
			...
			var f = function() {
				var d = {};
				if (g.$) {
					d.top = g.offset.top ? (g.$.offset().top + g.offset.top)
							: (g.$.offset().top + g.$.height());
					d.left = g.offset.left ? g.$.offset().left + g.offset.left
							: g.$.offset().left
				} else {
					d.top = parseInt(g.$window.height() - g.h) / 2
							+ g.scrollHeight;
		...
		autoClose : function(e, f) {
			var c = this, e = e || c.times, f = f || c.auto;
			if (f) {
...
(function(a) {
	a.fn.pageList = function() {
		...
		var A = b.totalNumber, D = b.pageNum, x = b.pageSize;
		var r = "", t = "", g = "", n = "", F = "", m = "", o = "", B = 0;
		var j = 0, h = 0, e = 0, c = 0, l = "", k = "", G = "", E = "", z = "", w = "", n = "";
		...
		if (r < q) {
			for (var y = 1; y <= r; y++) {
				if (y == D) {
					...
					n += '<a href="javascript:void(0)" info="' + y + '">' + y
							+ "</a>"
				}
			}
		} else {
			if (D <= 1) {
				for (var y = 1; y <= 3; y++) {
					if (y == D) {
						...
						n += '<a href="javascript:void(0)" info="' + y + '">'
								+ y + "</a>"
					}
				}
				...
				if (r - D <= 1) {
					...
					e = D + 1;
					c = D + 2;
					l = j <= 1 ? "" : '<a href="javascript:void(0)" info="' + j
							+ '">' + j + "</a>";
					k = h <= 1 ? "" : '<a href="javascript:void(0)" info="' + h
							+ '">' + h + "</a>";
					G = e >= r ? "" : '<a href="javascript:void(0)" info="' + e
							+ '">' + e + "</a>";
					E = c >= r ? "" : '<a href="javascript:void(0)" info="' + c
							+ '">' + c + "</a>";
		...
		if (r - D > 0 && D == 1) {
			F = D + 1;
			u.append('<span class="selected">上页</span> ' + n
					+ ' <a href="javascript:void(0)" info="' + F + '">下页</a>')
		} else {
			if (r - D > 0 && D > 1) {
				F = D + 1;
				m = D - 1;
				u.append('<a href="javascript:void(0)" info="' + m
						+ '">上页</a> ' + n
						+ ' <a href="javascript:void(0)" info="' + F
						+ '">下页 </a>')
			} else {
				if (r - D == 0 && r > 1) {
					m = D - 1;
