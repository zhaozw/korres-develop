...
<div class="navbar navbar-inverse">
		  <div class="navbar-inner">
		   <div class="container-fluid">
			  <a class="brand" href="login_index.htm"><small><i class="icon-leaf"></i> ${pdm.SYSNAME}</small> </a>
			  <ul class="nav ace-nav pull-right">
					...
					<li class="light-blue user-profile">
						...
						<ul id="user_menu" class="pull-right dropdown-menu dropdown-yellow dropdown-caret dropdown-closer">
							...
							<li class="divider"></li>
							<li><a href="logout.htm"><i class="icon-off"></i> 退出</a></li>
						</ul>
					</li>
			  </ul><!--/.ace-nav-->
		   </div><!--/.container-fluid-->
		...
		<script type="text/javascript">
			...
			$(function(){
				...
				$("#skin-colorpicker").ace_colorpicker().on("change",function(){
					...
					hf(b);
					var url = '<%=basePathh%>/head/setSKIN.htm?SKIN='+b+'&tm='+new Date().getTime();
			...
			function getUname(){
				$.ajax({
					type: "POST",
					url: '<%=basePathh%>/head/getUname.htm?tm='+new Date().getTime(),
			    	data: encodeURI(""),
					dataType:'json',
			...
			function editUserH(){
				//alert(USER_ID);
				 ...
				 diag.Drag=true;
				 diag.Title ="个人资料";
				 diag.URL = '<%=basePathh%>/user/goEditU.htm?USER_ID='+USER_ID+'&fx=head';
				 diag.Width = 225;
				 diag.Height = 389;
			...
			function editSys(){
				 ...
				 diag.Drag=true;
				 diag.Title ="系统设置";
				 diag.URL = '<%=basePathh%>/head/goSystem.htm';
				 diag.Width = 600;
				 diag.Height = 596;
				 diag.CancelEvent = function(){ //关闭事件
					diag.close();
			...
			function productCode(){
				 ...
				 diag.Drag=true;
				 diag.Title ="代码生成器";
				 diag.URL = '<%=basePathh%>/head/goProductCode.htm';
				 diag.Width = 800;
				 diag.Height = 450;
				 diag.CancelEvent = function(){ //关闭事件
					diag.close();
			...
			function zidian(){
				 ...
				 diag.Drag=true;
				 diag.Title ="数据字典";
				 diag.URL = '<%=basePathh%>/zidian.htm?PARENT_ID=0';
				 diag.Width = 799;
				 diag.Height = 460;
				 diag.CancelEvent = function(){ //关闭事件
					diag.close();
			...
			//切换菜单
			function changeMenu(){
				window.location.href='<%=basePathh%>/login_index.htm?&isM1=index&changeMenu=yes';
			}
