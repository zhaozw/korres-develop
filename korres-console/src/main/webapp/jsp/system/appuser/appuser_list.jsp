...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="happuser/listUsers.htm?isM1=${pdm.isM1 }&isM2=${pdm.isM2 }" method="post" name="userForm" id="userForm">
			<table border='0'>
				<tr>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function sendEmail(EMAIL){
			 ...
			 diag.Drag=true;
			 diag.Title ="发送电子邮件";
			 diag.URL = '<%=basePath%>/head/goSendEmail.htm?EMAIL='+EMAIL+'&msg=appuser';
			 diag.Width = 600;
			 diag.Height = 470;
		...
		function delUser(userId,msg){
			bootbox.confirm("确定要删除["+msg+"]吗?", function(result) {
				if(result) {
					var url = "happuser/deleteU.htm?USER_ID="+userId+"&tm="+new Date().getTime();
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							$.ajax({
								type: "POST",
								url: 'happuser/deleteAllU.htm?tm='+new Date().getTime(),
						    	data: {USER_IDS:str},
								dataType:'json',
		...
		function toExcel(){
			...
			window.location.href='<%=basePath%>/happuser/excel.htm?USERNAME='+USERNAME+'&lastLoginStart='+lastLoginStart+'&lastLoginEnd='+lastLoginEnd+'&ROLE_ID='+ROLE_ID+'&STATUS='+STATUS;
		}
		</script>
