...
<div>
				...
				<td style="width:69px;"><a href="javascript:addRole();" class="btn btn-small btn-success">新增</a></td>
					...
					<c:forEach items="${roleList}" var="role" varStatus="vs">
						<td style="width:100px;" class="center" <c:choose><c:when test="${pd.ROLE_ID == role.ROLE_ID}">bgcolor="#FFC926" onMouseOut="javascript:this.bgColor='#FFC926';"</c:when><c:otherwise>bgcolor="#E5E5E5" onMouseOut="javascript:this.bgColor='#E5E5E5';"</c:otherwise></c:choose>  onMouseMove="javascript:this.bgColor='#FFC926';" >
							<a href="role.htm?ROLE_ID=${role.ROLE_ID }&isM1=${pdm.isM1 }&isM2=${pdm.isM2 }" style="text-decoration:none; display:block;"><li class=" icon-group"></li>&nbsp;<font color="#666666">${role.ROLE_NAME }</font></a>
						</td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function addRole(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增部门";
			 diag.URL = 'role/toAdd.htm?parent_id=0';
			 diag.Width = 222;
			 diag.Height = 90;
		...
		function addRole2(pid){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增职位";
			 diag.URL = 'role/toAdd.htm?parent_id='+pid;
			 diag.Width = 222;
			 diag.Height = 90;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
		...
		function editRole(ROLE_ID){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = 'role/toEdit.htm?ROLE_ID='+ROLE_ID;
			 diag.Width = 222;
			 diag.Height = 90;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
		...
		function delRole(ROLE_ID,msg,ROLE_NAME){
			bootbox.confirm("确定要删除["+ROLE_NAME+"]吗?", function(result) {
				if(result) {
					var url = "role/delete.htm?ROLE_ID="+ROLE_ID+"&guid="+new Date().getTime();
					$.get(url,function(data){
						if(data=="success"){
							if(msg == 'c'){
								document.location.reload();
							}else{
								window.location.href="role.htm?1=1&isM1=1&isM2=2";
							}
		...
		function kf_qx1(id,kefu_id,msg){
			...
			if(qxhc1 == 'checked'){
				...
				var url = "role/kfqx.htm?kefu_id="+kefu_id+"&msg="+msg+"&value="+value+"&guid="+new Date().getTime();
				$.get(url,function(data){
					if(data=="success"){
						//document.location.reload();
					}
		...
		function c1(id,msg,value,kefu_id){
				...
				var url = "role/gysqxc.htm?kefu_id="+kefu_id+"&msg="+msg+"&value="+value+"&guid="+new Date().getTime();
				$.get(url,function(data){
					if(data=="success"){
						//document.location.reload();
					}
		...
		function editRights(ROLE_ID){
			 ...
			 diag.Drag = true;
			 diag.Title = "菜单权限";
			 diag.URL = 'role/auth.htm?ROLE_ID='+ROLE_ID;
			 diag.Width = 280;
			 diag.Height = 370;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
		...
		function roleButton(ROLE_ID,msg){
			...
			if(msg == 'add_qx'){
			 ...
			 diag.Drag = true;
			 diag.Title = Title;
			 diag.URL = 'role/button.htm?ROLE_ID='+ROLE_ID+'&msg='+msg;
			 diag.Width = 200;
			 diag.Height = 370;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
